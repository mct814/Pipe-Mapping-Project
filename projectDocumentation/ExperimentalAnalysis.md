...
Secondly, the current IMU has no way to implement zero angular velocity update, which would allow us to set the gyroscope values to zero when the device is not moving. Due to this we expected to have an amount of drift but the values seen are a lot stronger than the values we estimated. One solution to this would be to implement a magnometer as well, but then the design would need to eliminate extra sources of electromagnetic interferance.
