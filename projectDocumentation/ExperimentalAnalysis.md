...
The error originates from four main causes, the first cause is due to the issue with the current probe. The current probe has many wires connected to each part, which causes the probe to move when we gather data.  We attempted to correct this by taping the wires down but there was still error. We then taped down the probe itself but again, due to the wires and the fact it has to be connected to a computer while gathering data, it has error as we could not get the probe perfectly straight, steady and level which causes both accelerometer and gyroscope error.
...
The third cause of error is the sampling time, since memory has been touchy, in response we lowered the sampling time for the memory to function better, but this comes with the cost of fewer data points per second. This causes the map to be less accurate as it doesn’t have as many points to map.
