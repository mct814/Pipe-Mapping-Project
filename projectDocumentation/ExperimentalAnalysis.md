...
Secondly, the current IMU has no way to implement zero angular velocity update on the device itself, which would allow us to calibrate the gyroscope to show zero when the device is not moving. Currently the gyroscope is showing low values that affect our analysis. We originally suspected this could be causing a significant amount of error. After doing more experiments with the data and manually calibrating the gyroscope we no longer suspect that this is a major issue, but it could still be causing some amount of error. The algorithm we are using calibrates the sensors manually, which is helpful, but the algorithm itself is out of the scope of the project, written as a PhD thesis, so we aren't fully sure what the "calibration" includes.
...
Even though the team has not found a specific cause of the error seen in the experimentation we believe our experiments were conducted in as detailed a manner as possible, and we have narrowed down possible causes of error to a smaller subset. We have tested the memory, the sensors, and the data analysis. Experiment 3 defines our results from testing the memory and the data integrity, all passing with no problems. The sensors have been tested using the IMU gyroscope and accelerometer data, however, testing these individually is a difficult task. Both sensors data is difficult to read and interpret in raw form so the final graph is needed to measure success. The data has even been checked against exmaple data provided by the open source project and shows no large deviations or noticable errors in the format. Since the final graph is dependent on both the sensors supplying data, the sensors could not be reliably separated and tested on their own. The data analysis software is open source and has been tested with multiple example files.

We have conducted a variety of experiments using different variations and parameters and continue to get similar results as displayed above. Due to the inability to narrow the results down by experimentation alone we believe this is an occasion where the experimentation tools need to be altered to understand the different results. Mainly, the experimentation needs to be conducted as it was first imagined. We have made various changes and accomodations to our original design to ensure the basics of our system worked as intended and we believe these changes have made it difficult to narrow down the results to a specific error. We currently do not have the ability to move the drone at a constant acceleration or speed, without severly putting the delicate wires at risk. We do not have the ability to error check our IMU data against another source to mitigate natural occurrences of noise and error. Finally, we are not able to put our system in an enclosed environment to avoid large variations in jostling and large oscillatory movements. 

This conclusion is not an attempt to blame our current state of the drone for our errors and lack of specificity in results, but to show that the team has exhausted all known methods used to pinpoint error in this complicated system. 
