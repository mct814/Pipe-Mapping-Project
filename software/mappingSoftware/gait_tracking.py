...
data = numpy.genfromtxt("Test5_Pipe1_Run2_Data.csv", delimiter=",", skip_header=0)

sample_rate = 52  # 400 Hz
# threshold = 0.118
threshold = 0.4
...
# for index in range(len(accelerometer)):
    # accelerometer[index, 1] = (accelerometer[index, 1]) * 1.6
    # accelerometer[index, 1] = numpy.abs(accelerometer[index, 1])
...
for index in range(len(timestamp)):
    is_moving[index] = numpy.sqrt(acceleration[index].dot(acceleration[index])) > threshold  # threshold = 3 m/s/s
...
# ax.set_xlabel("m (x-axis)")
# ax.set_ylabel("m (y-axis)")
# ax.set_zlabel("m (z-axis)")
# ax.scatter3D(position[:, 0], position[:, 1], position[:, 2], color="green")
# pyplot.title("Figure 1. Pipe 1 Run 1")

# xbb = numpy.concatenate((numpy.linspace(0, -9.75, len(timestamp) // 2), numpy.linspace(-9.75, -9.75, len(timestamp) // 2)), axis=None)
# ybb = numpy.concatenate((numpy.linspace(0, 0, len(timestamp) // 2), numpy.linspace(0, 0, len(timestamp) // 2)), axis=None)
# zbb = numpy.concatenate((numpy.linspace(0, 0, len(timestamp) // 2), numpy.linspace(0, 0, len(timestamp) // 2)), axis=None)

xbb = numpy.linspace(0, 0, len(timestamp))
ybb = numpy.linspace(0, 0, len(timestamp))
zbb = numpy.linspace(0, .00001, len(timestamp))

# ax.scatter3D(xbb, ybb, zbb, color="blue")

print(position[len(timestamp) - 1, 0] - xbb[len(timestamp) - 1])
print(position[len(timestamp) - 1, 1] - ybb[len(timestamp) - 1])
print(position[len(timestamp) - 1, 2] - zbb[len(timestamp) - 1])
...
if True:
    ...
    xb = numpy.linspace(0, 0, len(timestamp) // 2)
    yb = numpy.linspace(0, 0, len(timestamp) // 2)
    zb = numpy.linspace(0, .00001, len(timestamp) // 2)

    # xb = numpy.concatenate((numpy.linspace(0, -9.75, len(timestamp) // 2), numpy.linspace(-9.75, -9.75, len(timestamp) // 2)), axis=None)
    # yb = numpy.concatenate((numpy.linspace(0, 0, len(timestamp) // 2), numpy.linspace(0, 0, len(timestamp) // 2)), axis=None)
    # zb = numpy.concatenate((numpy.linspace(0, 0, len(timestamp) // 2), numpy.linspace(0, 0, len(timestamp) // 2)), axis=None)


    scatter = axes.scatter(x, y, z)

    axes.scatter3D(xb, yb, zb)
